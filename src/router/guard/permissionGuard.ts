...
const PRODUCT_PATH = PageEnum.PRODUCT_PAGE;
// const ROOT_PATH = RootRoute.path;

const whitePathList: PageEnum[] = [LOGIN_PATH, SSO_LOGIN_PATH, PRODUCT_PATH];
...
export function createPermissionGuard(router: Router) {
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    if (!token) {
      ...
      const queryData: Recordable<string | string[]> = {};
      queryKeys.forEach((key) => {
        if (to.query[key] !== undefined && to.query[key] !== null) {
          queryData[key] = to.query[key]!.toString();
        }
      });
      // redirect login page
      const redirectData: {
        path: string;
        replace: boolean;
        query?: Recordable<string | string[]>;
      } = {
        path: LOGIN_PATH,
        replace: true,
        query: queryData,
    ...
    if (to.name === PAGE_NOT_FOUND_ROUTE.name) {
      ...
      if (redirect === to.fullPath) {
        ...
        // 指向redirect
        next({ path: redirect, replace: true, query: from.query });
      }
    } else {
      // 正常访问
      next();
